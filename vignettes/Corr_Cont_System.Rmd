...
1. $X_{11}, X_{21}, X_{31}$ have Exponential(2) distributions    

2. $X_{12}, X_{22}, X_{32}$ have Laplace(0, 1) distribution s 

3. $E_1, E_2, E_3$ have Cauchy(0, 1) distributions (the standardized cumulants were taken from the paper)     
...
1. $X_{11} = X_{21} = X_{31}$ has a Logistic(0, 1) distribution and is the same variable for each Y 

2. $X_{12}, X_{22}, X_{32}$ have Beta(4, 1.5) distributions 

3. $X_{13}, X_{23}, X_{33}$ have normal mixture distributions 
...
Sincere the system contains mixture variables, the mixture parameter inputs `mix_pis`, `mix_mus`, and `mix_sigmas` and the `error_type` must also be used.

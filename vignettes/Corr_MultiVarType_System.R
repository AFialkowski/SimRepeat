...
#  rand.int <- "non_mix"
#  rand.tsl <- "non_mix"
#  rand.var <- matrix(c(1, 3, 2, 3, 3, 3), 3, 2, byrow = TRUE)
#  
#  Stcum5 <- calc_theory("Logistic", c(0, 1))
#  Stcum6 <- calc_theory("t", 10)
...
#  rSix <- lapply(seq_len(M), function(x) list(1.75, NULL))
#  
#  rmix_pis <- lapply(seq_len(M), function(x) list(c(0.3, 0.2, 0.5)))
#  rmix_mus <- lapply(seq_len(M), function(x) list(c(L[1], C[1], B[1])))
#  rmix_sigmas <- lapply(seq_len(M), function(x) list(c(L[2], C[2], B[2])))
#  rmix_skews <- lapply(seq_len(M), function(x) list(c(L[3], C[3], B[3])))
#  rmix_skurts <- lapply(seq_len(M), function(x) list(c(L[4], C[4], B[4])))
#  rmix_fifths <- lapply(seq_len(M), function(x) list(c(L[5], C[5], B[5])))
#  rmix_sixths <- lapply(seq_len(M), function(x) list(c(L[6], C[6], B[6])))
#  rmix_Six <- lapply(seq_len(M), function(x) list(1.75, NULL, 0.03))
#  Mstcum <- calc_mixmoments(rmix_pis[[1]][[1]], rmix_mus[[1]][[1]],
#    rmix_sigmas[[1]][[1]], rmix_skews[[1]][[1]], rmix_skurts[[1]][[1]],
#    rmix_fifths[[1]][[1]], rmix_sixths[[1]][[1]])
#  
#  rmeans <- lapply(seq_len(M), function(x) c(Stcum5[1], Stcum6[1], Mstcum[1]))
#  rvars <- lapply(seq_len(M), function(x) c(Stcum5[2]^2, Stcum6[2]^2,
#    Mstcum[2]^2))
#  
#  # append parameters for random effect distributions to parameters for
...
#  sixths <- append(sixths, rsixths)
#  Six <- append(Six, rSix)
#  mix_pis <- append(mix_pis, rmix_pis)
#  mix_mus <- append(mix_mus, rmix_mus)
#  mix_sigmas <- append(mix_sigmas, rmix_sigmas)
#  mix_skews <- append(mix_skews, rmix_skews)
#  mix_skurts <- append(mix_skurts, rmix_skurts)
#  mix_fifths <- append(mix_fifths, rmix_fifths)
#  mix_sixths <- append(mix_sixths, rmix_sixths)
#  mix_Six <- append(list(NULL, NULL, NULL), rmix_Six)
#  
#  corr.u <- lapply(seq_len(M), function(x) lapply(seq_len(M),
#    function(x) matrix(0.4, 5, 5)))
#  diag(corr.u[[1]][[1]]) <- diag(corr.u[[2]][[2]]) <-
#    diag(corr.u[[3]][[3]]) <- 1
#  # set correlations between components of same mixture variable to 0
#  corr.u[[1]][[1]][3:5, 3:5] <- corr.u[[2]][[2]][3:5, 3:5] <-
#    corr.u[[3]][[3]][3:5, 3:5]
